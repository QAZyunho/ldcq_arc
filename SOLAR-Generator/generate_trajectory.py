    ...
    for t in ['train','test']:
        if save_seg_trace == "true" or save_whole_trace == "true":
            if delete_existing_data != "true":
                ...
                whole_folder_path = data_folder_path+'/whole'+f"/{tasks[i]}.s{max_grid_dim[0]}.{formatted_time}"
                ...
                seg_folder_path = data_folder_path+'/segment'+f"/{tasks[i]}.s{max_grid_dim[0]}.{formatted_time}"
                ...
                wrong_folder_path = data_folder_path+'/wrong'+f"/{tasks[i]}.s{max_grid_dim[0]}.{formatted_time}"
                if os.path.exists(wrong_folder_path):
                    shutil.rmtree(wrong_folder_path)

        try:
            if t=='test':
                grid_maker = utils.import_library_for_task(tasks[i], 100, max_grid_dim, num_examples=num_examples, rand_seed=rand_seed+5)
            else:
        ...
        for j in tqdm(range(len(grid_maker.data)), desc=f"task-{tasks[i]}-{t}", position=1):
            ...
            if 'gold_standard' in data['desc'] and not np.array_equal(np.array(data['grid'][-1])[:g_h, :g_w], pr_out[0]):
            ...
            if save_whole_trace == "true":
                utils.save_whole(data, tasks[i], t, max_grid_dim[0], data_folder_path)
