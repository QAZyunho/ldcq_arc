    ...
    with torch.no_grad():
        ...
        for eval_step in pbar:
            ...
            for env_idx in range(len(envs)):
                ...
                # test input 추출
                obs, info = envs[env_idx].reset(options={'prob_index': eval_step, 'subprob_index': 0})

                obs_x, obs_y = obs['grid_dim']
                clip_x, clip_y = obs['clip_dim']
                
                state_0[env_idx, :obs_x, :obs_y] = torch.from_numpy(obs['grid'][:obs_x, :obs_y]).to(args.device)
                clip_0[env_idx, :clip_x, :clip_y] = torch.from_numpy(obs['clip'][:clip_x, :clip_y]).to(args.device)
                in_grid[env_idx, :obs_x, :obs_y] = torch.from_numpy(obs['grid'][:obs_x, :obs_y]).to(args.device)
                    ...
                    for env_idx in range(len(envs)):
                        ...
                        if not done[env_idx]:
                            ...
                            operation, x, y, h, w = skill_model.decoder.ll_policy.tensor_policy(
                                state_0[env_idx].unsqueeze(0), clip_0[env_idx].unsqueeze(0), in_grid[env_idx].unsqueeze(0), best_latent[env_idx], pair_in[env_idx].unsqueeze(0), pair_out[env_idx].unsqueeze(0))
                            
                            operation = torch.argmax(operation).cpu().numpy()  # argmax 후 CPU로 옮겨 NumPy로 변환하고 스칼라로 만듦
                            x = torch.argmax(x).cpu().numpy()  # argmax 후 NumPy 배열로 변환
                            y = torch.argmax(y).cpu().numpy()  # argmax 후 NumPy 배열로 변환
                            h = torch.argmax(h).cpu().numpy()  # argmax 후 NumPy 배열로 변환
                            w = torch.argmax(w).cpu().numpy() 
                            ...
                            if(operation == 35):
                                print("None !!")
                                ValueError("잘하자")
                            # elif(sample_num < args.num_diffusion_samples-1):
                            #     pass
                            else:
                                # time.sleep(1.0)
                                ...
                                state_0[env_idx, :obs_x, :obs_y] = torch.from_numpy(obs['grid'])
                                
                                clip_x, clip_y  = obs['clip_dim']
                                clip_0[env_idx].fill_(10)
                                clip_0[env_idx, :clip_x, :clip_y] = torch.from_numpy(obs['clip'])

                                if np.array_equal(obs['grid'], tt_out[:obs_x, :obs_y]):
            ...
            print(f'Total score: {score_reach} out of {total_runs} i.e. Acc: {(score_reach / total_runs) * 100}%')
